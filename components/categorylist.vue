    ...
    methods: {
      async fetchMovies() {
        const urls = [
          ...
          `https://api.themoviedb.org/3/discover/movie?api_key=2e51d2298001e0dddf6a6ea7553cd801&with_genres=10749`,
        ...
        try {
          const [fantasyMovies, horrorMovies, romanceMovies, thrillerMovies] = await Promise.all(fetchPromises);
    
          this.categories[0].movies = fantasyMovies.results;
          this.categories[1].movies = horrorMovies.results;
          this.categories[2].movies = romanceMovies.results;
          this.categories[3].movies = thrillerMovies.results;
        } catch (error) {
